...
def menu(request):
    # Consumir la API de TheMealDB
    response = requests.get('https://www.themealdb.com/api/json/v1/1/filter.php?c=Seafood')
    meals = response.json().get('meals', [])
    
    # Generar precios y descripciones aleatorias
    for meal in meals:
        meal['price'] = round(random.uniform(5, 30), 2)  # Precio entre 5 y 30
        meal['description'] = "Delicious and freshly prepared."  # Descripción inventada
    
    context = {
        'meals': meals,
        'form': LoginForm(),
    }
    return render(request, 'bodega/menu.html', context)
